...
* Import delimited
import delimited "muestra_1_10porciento.csv", clear
...
* Create dummy for cambio_sal
gen sal_cierre_aux = sal_cierre[_n-1]
gen cambio_sal = [cambio_cierre == 1 & sal_cierre_aux > sal_cierre]
...
bysort idnss: egen sal_prom_prev_aux = mean(sal_cierre) if periodo_year == 2020
bysort idnss: egen sal_prom_prev = max(sal_prom_prev_aux)
xtile decile = sal_prom_prev, nq(10)

* Drop auxiliar dummies
drop baja baja_aux idnss_aux idrfc_aux sal_cierre_aux max_periodo_alta max_periodo_alta_aux sal_prom_prev_aux
...
balancetable treated sexo sal_cierre gobierno outsourcing ent* div* using "Panel_RPCI_Balance_Table.xlsx", replace
...
********************************************
* Industry & Quarter Interaction Dummies *
********************************************
...
foreach depvar in "sal_cierre" { // "log_sal_cierre" "baja_cierre" "baja_permanente" "cambio_cierre"
	...
	reghdfe `depvar' _ItreXtim_1_`lo'-_ItreXtim_1_`=`increment'-2' 
		...
		_ItreXtim_1_`increment'-_ItreXtim_1_`hi'
			/* event study relative time dummies starting at k=0 */, 
			absorb(periodo idnss i.div_final##i.periodo_quarter)
	;
	#delimit cr
	// Note the coefficients we want from the regression are 
	//  the coefficients on _ItreXtim_1_*.
	...
	* Export the graph
	graph export "event_study_`depvar'_months_max_div.pdf", replace
...
*****************************************************************
* State, Industry and Wage Decile & Quarter Interaction Dummies *
*****************************************************************


foreach depvar in "sal_cierre" { // "baja_cierre" "baja_permanente" "cambio_cierre"
	preserve
	...
	reghdfe `depvar' _ItreXtim_1_`lo'-_ItreXtim_1_`=`increment'-2' 
		...
		_ItreXtim_1_`increment'-_ItreXtim_1_`hi'
			/* event study relative time dummies starting at k=0 */, 
			absorb(periodo idnss i.cve_ent_final##i.periodo_quarter i.div_final##i.periodo_quarter i.decile##i.periodo_quarter)
	;
	#delimit cr
	// Note the coefficients we want from the regression are 
	//  the coefficients on _ItreXtim_1_*.
	...
	* Export the graph
	graph export "event_study_`depvar'_months_max_ent_div_sal_decil.pdf", replace
...
******************************************************************************
* State, Industry and Wage Decile & Quarter Interaction Dummies, Outsourcing *
******************************************************************************
...
foreach depvar in "sal_cierre" { // "baja_cierre" "baja_permanente" "cambio_cierre"
	...
	reghdfe `depvar' _ItreXtim_1_`lo'-_ItreXtim_1_`=`increment'-2' 
		...
		_ItreXtim_1_`increment'-_ItreXtim_1_`hi'
			/* event study relative time dummies starting at k=0 */, 
			absorb(periodo idnss i.cve_ent_final##i.periodo_quarter i.div_final##i.periodo_quarter i.decile##i.periodo_quarter outsourcing)
	;
	#delimit cr
	// Note the coefficients we want from the regression are 
	//  the coefficients on _ItreXtim_1_*.
	...
	* Export the graph
	graph export "event_study_`depvar'_months_max_ent_div_sal_decil_outsourcing.pdf", replace
	
	restore
}

******************************************************************************
* State, Industry and Wage Decile & Quarter Interaction Dummies, Num_workers *
******************************************************************************
...
foreach depvar in "sal_cierre" { // "baja_cierre" "baja_permanente" "cambio_cierre"
	...
	reghdfe `depvar' _ItreXtim_1_`lo'-_ItreXtim_1_`=`increment'-2' 
		...
		_ItreXtim_1_`increment'-_ItreXtim_1_`hi'
			/* event study relative time dummies starting at k=0 */, 
			absorb(periodo idnss i.cve_ent_final##i.periodo_quarter i.div_final##i.periodo_quarter i.decile##i.periodo_quarter num_workers)
	;
	#delimit cr
	// Note the coefficients we want from the regression are 
	//  the coefficients on _ItreXtim_1_*.
	...
	* Export the graph
	graph export "event_study_`depvar'_months_max_ent_div_sal_decil_num_workers.pdf", replace
...
******************************************************************************
* State, Industry and Wage Decile & Quarter Interaction Dummies, Outsourcing, num_workers *
******************************************************************************
...
foreach depvar in "sal_cierre" { // "baja_cierre" "baja_permanente" "cambio_cierre"
	...
	reghdfe `depvar' _ItreXtim_1_`lo'-_ItreXtim_1_`=`increment'-2' 
		...
		_ItreXtim_1_`increment'-_ItreXtim_1_`hi'
			/* event study relative time dummies starting at k=0 */, 
			absorb(periodo idnss i.cve_ent_final##i.periodo_quarter i.div_final##i.periodo_quarter i.decile##i.periodo_quarter num_workers outsourcing)
	;
	#delimit cr
	// Note the coefficients we want from the regression are 
	//  the coefficients on _ItreXtim_1_*.
	...
	* Export the graph
	graph export "event_study_`depvar'_months_max_ent_div_sal_decil_outsourcing_num_workers.pdf", replace
