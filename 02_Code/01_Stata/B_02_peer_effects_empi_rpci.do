...
***********************************
* Effect on RPCI register: LAGGED *
***********************************

* Specification: rpci_it = \alpha + \beta * rfc_rpci_it-1 + \varepsilon
	
eststo: reghdfe rpci rfc_rpci_dum_lag, absorb(idrfc periodo idnss) cluster(idnss)
gen reg_sample = [e(sample) == 1]
...
* Specification: rpci_it = \alpha + \beta * rfc_rpci_it-1 + \gamma * perc_rpci_exclu_it-1 + \varepsilon

eststo: reghdfe rpci rfc_rpci_dum_lag perc_rpci_exclu_lag, absorb(idrfc periodo idnss) cluster(idnss)
gen reg_sample = [e(sample) == 1]
...
* Specification: rpci_it = \alpha + \beta * rfc_rpci_vig_it-1 + \varepsilon
	
eststo: reghdfe rpci rfc_rpci_vig_dum_lag, absorb(idrfc periodo idnss) cluster(idnss)
gen reg_sample = [e(sample) == 1]
...
* Specification: rpci_it = \alpha + \beta * rfc_rpci_it-1 + \gamma * perc_rpci_vig_exclu_it-1 + \varepsilon

eststo: reghdfe rpci rfc_rpci_vig_dum_lag perc_rpci_vig_exclu_lag, absorb(idrfc periodo idnss) cluster(idnss)
gen reg_sample = [e(sample) == 1]
...
esttab using "03_Tables/$muestra/peer_rpci_lag.tex", replace b(`dec_b') se(`dec_se') $stars nolines nomtitle ///
stats(N dep_mean unique_idnss unique_idrfc fe idrfc_fe time_fe idnss_fe, fmt(%12.0fc %12.3fc %12.0fc %12.0fc) labels("\midrule Observations" "Dep. Var. Mean" "Workers" "Firms" "\midrule \emph{FE}" "\hspace{0.25cm}Firm" "\hspace{0.25cm}Period" "\hspace{0.25cm}Worker")) substitute("\_" "_")
eststo clear
