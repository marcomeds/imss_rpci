...
foreach depvar in `vars'{
	...
	forvalues s = 0(1)18 {	
		...
		***********************************************************
		...
		reghdfe `depvar' rpci_vig if download_monthly == 0 | download_monthly == tm(2021m2) + `s', ///
...
*****************************************************
* TWFE betas by cohort restricted to 6 months after *
*****************************************************
	
matrix results = J(15, 5, .)
...
foreach depvar in `vars'{
	...
	twoway 	(scatter beta cohort, color("0 69 134"))  ///
		...
		(rcap rcap_lo_10 rcap_hi_10 cohort, lwidth(thick) lcolor("0 69 134"%70)), ///
		legend(off) scheme(s2mono) graphregion(color(white)) ///
		tlabel(2021m2(1)2022m4, format(%tmMon-YY) angle(45)) ///
		xtitle("Month the worker registered for the RPCI" "(Cohort)") ytitle("Average causal effect after 6 months") yline(0)
		
	graph export "04_Figures/$muestra/twfe_beta_cohort_`depvar'_6m.pdf", replace	
